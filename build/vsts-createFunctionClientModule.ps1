...
$parentModule = 'Principal'
if (-not $ModuleName) { $ModuleName = 'Principal.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Principal'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Principal.psd1"
$prereqHash = @{
